...
public class StudentSubmissionPane implements Runnable {
    ...
    public StudentSubmissionPane(String username, String password) {
        ...
        public void actionPerformed(ActionEvent e) {
        	...
        	for (int j = 0; j < count; j++) {
        		if (e.getSource() == buttons.get(j)) {
        			try {
		                frame.dispatchEvent(new WindowEvent(frame, WindowEvent.WINDOW_CLOSING));
		                new Thread(new SubmissionPane(username, password, submissionNumbers.get(j))).start();
        			} catch (Exception ex) {
        				ex.printStackTrace();
        			}
        		}
