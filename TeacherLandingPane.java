...
public class TeacherLandingPane implements Runnable {
    ...
    public void run() {
        try {
            ...
            //quiz button
            gradeCenter = new JButton("Grade Center");
            gradeCenter.setFont(new Font("Calicanto-SemiBold", Font.BOLD, 32));
            c.fill = GridBagConstraints.HORIZONTAL;
            c.weightx = 0.5;
            c.gridx = 0;
            c.gridy = 3;
            c.ipady = 100;
            c.ipadx = 100;
            gradeCenter.addActionListener(actionListener);
            pane.add(gradeCenter, c);
            ...
            //submission button
            quizCenter = new JButton("Quiz Center");
            quizCenter.setFont(new Font("Calicanto-SemiBold", Font.BOLD, 32));
            c.fill = GridBagConstraints.HORIZONTAL;
            c.weightx = 0.5;
            c.gridx = 0;
            c.gridy = 4;
            c.ipady = 100;
            c.ipadx = 100;
            quizCenter.addActionListener(actionListener);
            pane.add(quizCenter, c);

            //welcome message
            JLabel name = new JLabel("Professor");
            c.fill = GridBagConstraints.REMAINDER;
            c.weightx = 0.5;
            c.gridx = 1;
            c.gridy = 4;
    ...
    ActionListener actionListener = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (e.getSource() == gradeCenter) {
                gradeCenter();
            }
            if (e.getSource() == quizCenter) {
                quizCenter();
            }
        }
    };
    ...
    public void gradeCenter() {
        frame.dispatchEvent(new WindowEvent(frame, WindowEvent.WINDOW_CLOSING));
        new Thread(new TeacherGradingLandingPane(username, password)).start();
    }

    public void quizCenter() {
        frame.dispatchEvent(new WindowEvent(frame, WindowEvent.WINDOW_CLOSING));
        new Thread(new TeacherQuizMainLandingPane(username, password)).start();
    }
    /*
