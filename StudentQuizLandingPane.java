...
public class StudentQuizLandingPane implements Runnable {
    ...
    public StudentQuizLandingPane(String username, String password) {
    ...
    public void run() {
        try {
            ...
            quizNumbers = findAvailableQuizzes();
            JButton quiz;
            c.gridwidth = 1;
            for (int i = 0; i < quizNumbers.size(); i++) {
            	...
            	c.gridx = i % 3;
            	c.insets = new Insets(10, 10, 10, 10);
            	quiz = new JButton("Quiz " + quizNumbers.get(i).toString());
        ...
        public void actionPerformed(ActionEvent e) {
        	...
        	for (int j = 0; j < count; j++) {
        		if (e.getSource() == buttons.get(j)) {
        			try {
        				takeQuiz(quizNumbers.get(j));
		                frame.dispatchEvent(new WindowEvent(frame, WindowEvent.WINDOW_CLOSING));
		                new Thread(new StudentQuizPane(username, password, selectedQuiz, 0, new ArrayList<String>(), 0, 0)).start();
        			} catch (Exception ex) {
        				ex.printStackTrace();
        			}
        		}
    ...
    public IntegerArrayList findAvailableQuizzes() throws IOException, ClassNotFoundException {
        Socket socket = new Socket("localhost", 4242);
        ObjectInputStream ois = new ObjectInputStream(socket.getInputStream());
        ObjectOutputStream oos = new ObjectOutputStream(socket.getOutputStream());
        oos.flush();
        IntegerArrayList quizzes = new IntegerArrayList();
        oos.writeObject("findAvailableQuizzes");
        oos.flush();
        oos.writeObject(this.username);
        oos.flush();
        quizzes = (IntegerArrayList) ois.readObject();
        return quizzes;
    }
